...
DATA_FABRIC_SOURCES: list[DataSourceSpec] = [
    DataSourceSpec(
        ...
        notes="Feeds Risk Agent and Digital Twin weather state.",
    ),
    DataSourceSpec(
        ...
        notes="NDVI, crop stress and field condition layers.",
    ),
    DataSourceSpec(
        ...
        notes="Wheat, corn, sunflower and regional trade references.",
    ),
    DataSourceSpec(
        name="FX Rates",
        kind=SourceKind.external,
        ...
        notes="EUR, USD, BGN exposure for trade and cost calculations.",
    ),
    DataSourceSpec(
        ...
        notes="Triggers autonomous purchase opportunity workflows.",
    ),
    DataSourceSpec(
        name="Fuel Prices",
        kind=SourceKind.external,
        ...
        target_store="clickhouse",
        freshness_sla_minutes=1440,
        event_topic="external.inputs.fuel_price.v1",
        notes="Feeds logistics and machine cost models.",
    ),
    DataSourceSpec(
        name="ERP",
        ...
        status=SourceStatus.planned,
        target_store="neo4j",
        event_topic="internal.erp.record.v1",
        notes="Farms, fields, employees, operations and contracts.",
    ),
    DataSourceSpec(
        name="Accounting",
        kind=SourceKind.internal,
        ...
        status=SourceStatus.planned,
        target_store="clickhouse",
        event_topic="internal.accounting.transaction.v1",
        notes="Costs, receivables, payables and cash-flow facts.",
    ),
    DataSourceSpec(
        name="Warehouses",
        kind=SourceKind.internal,
        ...
        status=SourceStatus.planned,
        target_store="neo4j",
        event_topic="internal.warehouse.inventory.v1",
        notes="Crop inventory, quality and available tonnage.",
    ),
    DataSourceSpec(
        name="GPS Systems",
        kind=SourceKind.internal,
        ...
        status=SourceStatus.planned,
        target_store="clickhouse",
        event_topic="internal.gps.position.v1",
        notes="Machine movement, field work and logistics traces.",
    ),
    DataSourceSpec(
        name="Machines",
        kind=SourceKind.internal,
        ...
        status=SourceStatus.stub,
        target_store="clickhouse",
        event_topic="internal.machine.telemetry.v1",
        notes="Machine health and utilization for Operations Agent.",
    ),
    DataSourceSpec(
        name="Input Norms",
        kind=SourceKind.internal,
        ...
        status=SourceStatus.planned,
        target_store="neo4j",
        event_topic="internal.cost.norm.v1",
        notes="Seed, fertilizer, fuel and labor standards by crop and field.",
    ),
]


GRAPH_NODES: list[GraphNodeSpec] = [
    GraphNodeSpec(label="Farm", required_properties=["id", "name", "country"]),
    GraphNodeSpec(label="Field", required_properties=["id", "area_ha", "soil_type"]),
    GraphNodeSpec(label="Crop", required_properties=["id", "type", "season"]),
    GraphNodeSpec(label="Yield", required_properties=["id", "tons_per_ha", "season"]),
    GraphNodeSpec(label="Cost", required_properties=["id", "category", "amount", "currency"]),
    GraphNodeSpec(label="Machine", required_properties=["id", "type", "status"]),
    GraphNodeSpec(label="Employee", required_properties=["id", "role"]),
    GraphNodeSpec(label="Contract", required_properties=["id", "counterparty", "value", "currency"]),
    GraphNodeSpec(label="Warehouse", required_properties=["id", "capacity_tons"]),
]

GRAPH_RELATIONSHIPS: list[GraphRelationshipSpec] = [
    GraphRelationshipSpec(start="Farm", relationship="HAS_FIELD", end="Field", description="Farm owns or operates a field."),
    GraphRelationshipSpec(start="Field", relationship="GROWS", end="Crop", description="Crop currently or historically grown on field."),
    GraphRelationshipSpec(start="Field", relationship="PRODUCED", end="Yield", description="Yield result for crop and season."),
    GraphRelationshipSpec(start="Field", relationship="INCURRED", end="Cost", description="Field-level cost basis."),
    GraphRelationshipSpec(start="Farm", relationship="OWNS", end="Machine", description="Owned or leased machine fleet."),
    GraphRelationshipSpec(start="Farm", relationship="EMPLOYS", end="Employee", description="Operational and management staff."),
    GraphRelationshipSpec(start="Farm", relationship="HAS_CONTRACT", end="Contract", description="Sales, purchase, rent or service contract."),
    GraphRelationshipSpec(start="Farm", relationship="OWNS", end="Warehouse", description="Storage and inventory location."),
    GraphRelationshipSpec(start="Warehouse", relationship="STORES", end="Crop", description="Crop inventory stored in warehouse."),
]
...
AGENT_MESH: list[AgentSpec] = [
    AgentSpec(
        name="Finance Agent",
        responsibility="Margins, cost base, cash flow, working capital and profitability.",
        data_dependencies=["Accounting", "Contracts", "FX Rates", "Input Norms"],
        emits=["finance.margin.updated", "finance.cashflow.warning"],
    ),
    AgentSpec(
        name="Risk Agent",
        responsibility="Climate, price, liquidity, counterparty and logistics risk.",
        data_dependencies=["Weather", "Satellite", "Commodity Exchanges", "Contracts"],
        emits=["risk.score.changed", "risk.alert.created"],
    ),
    AgentSpec(
        name="Market Agent",
        responsibility="Global markets, exchange prices, demand, inputs and timing.",
        data_dependencies=["Commodity Exchanges", "FX Rates", "Fertilizer Prices", "Fuel Prices"],
        emits=["market.signal.created", "market.opportunity.detected"],
    ),
    AgentSpec(
        name="Operations Agent",
        responsibility="Machines, personnel, field work, capacity and bottlenecks.",
        data_dependencies=["ERP", "GPS Systems", "Machines", "Warehouses"],
        emits=["operations.bottleneck.detected", "operations.capacity.updated"],
    ),
    AgentSpec(
        name="Compliance Agent",
        responsibility="Regulations, subsidies, deadlines and documentation risk.",
        data_dependencies=["ERP", "Contracts", "Regulatory calendar"],
        emits=["compliance.deadline.warning", "compliance.requirement.created"],
    ),
    AgentSpec(
        name="Strategy Agent",
        responsibility="Synthesizes all agents into decisions and recommended actions.",
        data_dependencies=["Finance Agent", "Risk Agent", "Market Agent", "Operations Agent", "Compliance Agent"],
